...
class UserHandler:
    ...
    def register(self, user_name, age, is_admin, perm):
        self.lock.acquire()
        user = User(user_name, Cart(), guest=False, age=age, is_admin=is_admin)
        try:
            users.add_user(user, perm)
            self.lock.release()
    ...
    def get_guest_unique_user_name(self, permissions):
        new_user_name = get_random_string(GUEST_NAME_LENGTH)
        while True:
            new_user_name = get_random_string(GUEST_NAME_LENGTH)
            try:
                users.add_user(User(new_user_name, 0, Cart()), permissions)
                break
            except:
                continue
