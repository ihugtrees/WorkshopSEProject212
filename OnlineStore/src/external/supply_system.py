...
class SupplySystem:
    ...
    def supply(self, buyer_information: dict) -> str:
        ...
        response = requests.post(url, data={"action_type": "supply", "name": buyer_information["name"],
                                            ...
                                            "country": buyer_information["country"], "zip": buyer_information["zip"]}, timeout=3)
    ...
    def cancel_supply(self, transaction_id: int) -> None:
        url = "https://cs-bgu-wsep.herokuapp.com/"
        response = requests.post(url=url, data={"action_type": "cancel_supply", "transaction_id": transaction_id}, timeout=3)
        if str(response.content) != "b'OK'":
            raise Exception("Handshake went wrong")
        response = requests.post(url, data={"action_type": "cancel_supply", "transaction_id": transaction_id}, timeout=3)
